...
func handleConnection(conn *ss.Conn, config *ss.Config, port string) {
	var host string
	...
	remote, err := ss.DialWithRawAddr(
		rawAddr,
		net.JoinHostPort(config.Server.(string), strconv.Itoa(config.ServerPort)),
		cipher)
	if err != nil {
...
func run(port, password string) {
	...
	for {
		...
		go handleConnection(ss.NewConn(conn, cipher.Copy()), config, port)
	}
}
...
func enoughOptions(config *ss.Config) bool {
	return config.Server != nil && config.ServerPort != 0 && config.Password != ""
}
