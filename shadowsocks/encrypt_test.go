...
func checkTable(t *testing.T, enc, dec tableCipher, encTarget, decTarget []byte, msg string) {
	const tbl_size = 256
	for i := 0; i < tbl_size; i++ {
		if encTarget[i] != enc[i] {
			t.Fatalf("%s: encrypt table error at index %d\n", msg, i)
		}
		if decTarget[i] != dec[i] {
...
func TestEncrypTable1(t *testing.T) {
	...
	enc, dec := newTableCipher("foobar!")
	checkTable(t, enc, dec, encTarget, decTarget, "Error for password foobar!")
}
...
func TestEncryptTable2(t *testing.T) {
	...
	enc, dec := newTableCipher("barfoo!")
	checkTable(t, enc, dec, encTarget, decTarget, "Error for password barfoo!")
}
...
func TestTableCipher(t *testing.T) {
	...
	if err != nil {
		t.Error("Should not get error generating table cipher")
	}
