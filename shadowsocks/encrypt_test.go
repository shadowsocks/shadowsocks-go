...
func TestEncrypTable1(t *testing.T) {
	...
	SetDefaultCipher("")
	cipher, _ := NewCipher("foobar!")
	tbl, _ := cipher.(*TableCipher)
...
func TestTableCipher(t *testing.T) {
	if err := SetDefaultCipher(""); err != nil {
		t.Error("Error setting default table cipher")
	}
	cipher, err := NewCipher("OpenSesame!")
	if err != nil {
		t.Error("Should not get error generating table cipher")
	}
	if _, ok := cipher.(*TableCipher); !ok {
...
func TestRC4Cipher(t *testing.T) {
	if err := SetDefaultCipher("rc4"); err != nil {
		t.Error("Error setting default rc4 cipher")
	}
	cipher, err := NewCipher("")
	if err == nil {
		t.Error("Should get error for empty key creating rc4 cipher")
	}
	cipher, err = NewCipher("Alibaba")
	if err != nil {
